...
- `user_id` (optional)
- `num_speakers` (optional, used when diarization enabled)
- `min_speakers` (optional, used when diarization enabled)
- `max_speakers` (optional, used when diarization enabled)
...
```json
{
  ...
  "speaker_text_raw": [{"speaker": "SPEAKER_00", "start": 0.0, "end": 1.4, "text": "hello"}],
  "language": "auto",
...
Список завершенных транскрибаций (пагинация по 50).

**Query**:

- `page` (optional, default `1`)
- `task_id` (optional, поиск по task_id)
- `user_id` (optional, поиск по user_id)
  ...
  "items": [
    {
      "task_id": "uuid",
      "user_id": "external-id",
      "created_at": "2026-02-15T10:34:12.123Z",
      "duration": 120.5,
      "processing_time": 45.2,
      "language": "auto"
    }
